...
/** Raw signals scraped from a client's website, fed to Claude for normalization. */
export interface SiteSignals {
  url: string;
  name: string;
  logo?: string;
  themeColor?: string;
  /** Colors ranked by frequency of appearance in CSS. */
  colors: string[];
  /** Font families ranked by frequency. */
  fonts: string[];
  /** A trimmed sample of visible copy, for tone-of-voice analysis. */
  textSample: string;
}
...
function extractColors(css: string): string[] {
  ...
  // Drop pure black/white and fully transparent — keep brand-y colors near the top.
...
function extractFonts(css: string): string[] {
  ...
  const families = decls
    ...
    .filter(
      (v) =>
        v &&
        !/^(inherit|initial|unset|sans-serif|serif|monospace|system-ui|-apple-system)$/i.test(
          v,
        ),
    );
...
/** Scrape HTML + linked CSS and distil the raw brand signals. */
export async function collectSiteSignals(rawUrl: string): Promise<SiteSignals> {
  ...
  // A few same-origin stylesheets (bounded).
  ...
  // Visible copy sample for tone of voice.
...
const BrandSchema = z.object({
  name: z.string().describe("The brand or company name."),
  colors: z.object({
    primary: z.string().describe("Main brand color as a hex value."),
    secondary: z.string(),
    accent: z.string(),
    background: z.string(),
    surface: z.string().describe("Subtle off-background for cards/sections."),
    text: z.string(),
    muted: z.string(),
    border: z.string(),
  }),
  typography: z.object({
    ...
    googleFonts: z
      .array(z.string())
      .describe("Google Font family names to load for heading/body."),
    baseSize: z.string().describe('Body font size, e.g. "16px".'),
    scale: z.number(),
    headingWeight: z.number(),
    bodyWeight: z.number(),
  }),
  toneOfVoice: z.object({
    summary: z.string().describe("One or two sentences describing the voice."),
...
Rules:
- Choose colors that are accessible and harmonious. The frequency-ranked list is a hint, not a mandate — pick a confident primary, a darker secondary, and a complementary accent. Ensure text has strong contrast on background.
...
Output ONLY a single JSON object, with no prose and no markdown fences, matching exactly this shape:
{
  "name": string,
  "colors": { "primary": hex, "secondary": hex, "accent": hex, "background": hex, "surface": hex, "text": hex, "muted": hex, "border": hex },
  "typography": { "headingFont": string, "bodyFont": string, "googleFonts": string[], "baseSize": string, "scale": number, "headingWeight": number, "bodyWeight": number },
  "toneOfVoice": { "summary": string, "adjectives": string[] }
}`;
...
function heuristicTokens(signals: SiteSignals): BrandTokens {
  ...
  return withDefaults({
    ...
    toneOfVoice: {
      summary: "Derived from the site without AI — refine as needed.",
      adjectives: ["clear", "professional"],
    },
  });
}
...
export async function extractBrand(rawUrl: string): Promise<{
  ...
  try {
    ...
    // Never hard-fail the UX: fall back to heuristics.
