...
/** Ask Claude to revise a single block's HTML based on a free-text comment. */
...
Instruction: ${input.comment}`;
  ...
  if (!parsed.html) {
    throw new Error("The model did not return updated HTML.");
  }
  return { html: parsed.html, note: parsed.note ?? "Updated." };
}
